...
description: >-
  End-to-end open-source contribution for any GitHub project: discover repos
  (stars, recent commits, issue close ratio, PR merge speed, release cadence),
  analyze the repo, write a scoped PR plan, implement it, open a GitHub PR,
  then write a PR summary from saved artifacts.
  Use when the user wants 搜索可贡献项目, 找开源项目, 开源贡献, star 1000,
  GitHub 搜仓库; pastes a GitHub URL for 开源贡献分析, PR 方向, good first issue,
  架构缺陷, 测试缺口, 文档缺口; asks for 技术方案, 按方案落地, 提交 PR with
  feat/fix/refactor/test/docs; or 产出 PR 总结, summary.md, 已提 PR 复盘,
  pastes a pull request URL / 之前提过的项目.
  Trigger even without the word skill. Do not use for Agent/LLM/MCP-specific
  architecture contributions (use agent-oss-contribute) or private product feature work.
compatibility: Requires Python 3, network access, and GitHub CLI (gh) logged in or GH_TOKEN/GITHUB_TOKEN.
---

# 开源贡献全流程

面向任意开源仓库的一条流水线，六个阶段。用户没点名时，从当前对话能判断走到哪一步，就只做那一步；不要搜完项目自动改代码，也不要没方案就提 PR，也不要提完 PR 自动写总结。
...
| 给了 `REPO_URL` / 找 PR 方向 / Issue 筛选 / 代码缺陷 | 阶段 1 分析 | [references/analysis.md](references/analysis.md) 和 [references/contribution-dimensions.md](references/contribution-dimensions.md) |
...
仓库若是 Agent / LLM / MCP 项目、且用户要的是 Agent 架构层贡献（规划、多 Agent、tool 路由等），改用 `agent-oss-contribute`。本 skill 始终按通用贡献维度分析，不切换到 Agent 七层。
...
| 技术栈 | 未给则用：Python、TypeScript、Go、Rust |
| 目标周期 | 未给则用：2 周 |
...
- `<owner>-<repo>` 取自 `REPO_URL`（如 `https://github.com/pallets/click` → `pallets-click`）。
...
- 阶段 0 必须跑 `scripts/discover_repos.py`，默认 `--domain any`，禁止用记忆编造仓库榜；必须传 `--artifact-root`；用户口头过滤条件必须译成脚本参数。
- 每阶段结束必须落盘（见「中间产物落盘」），禁止只回复不写文件。
- 先读该仓库的真实文件和 Issue，再用训练记忆。路径用仓库内相对路径。
- 贡献范围必须能在约 2 周内完成，禁止「重写核心架构」这类方向。
